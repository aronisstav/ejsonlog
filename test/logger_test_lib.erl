...
setup(Config,Vars) ->
    ...
    case test_server:start_node(Sname, slave,
        ...
        send_files(Node, [ejsonlog, jiffy, jiffy_utf8]),
            ct:log("~p",[L]),
            {ok, L, Node};
        {error, Reason} ->
...
send_files(Node, Modules) ->
  Foreach =
    fun(M) ->
        {M, Bin, F} = code:get_object_code(M),
        rpc:call(Node, code, load_binary, [M, F, Bin]),
        ct:log("~p", [rpc:call(Node, M, module_info, [])]),
        ok
    end,
  lists:foreach(Foreach, Modules).
